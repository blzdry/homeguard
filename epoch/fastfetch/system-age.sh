_birth() {
    # Check if the repository file exists and output its change/creation time as a Unix timestamp
    if [ -f /var/db/repos/gentoo/profiles/repo_name ]; then
        stat -c %Z /var/db/repos/gentoo/profiles/repo_name 2>/dev/null
    # Fallback to Portage log first entry if the repository profile metadata changed
    elif [ -f /var/log/emerge.log ]; then
        head -n 1 /var/log/emerge.log | cut -d':' -f1 2>/dev/null
    fi
}
